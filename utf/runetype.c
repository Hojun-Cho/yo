...
static Rune *bsearch_(Rune c, Rune *t, int n, int ne)
{
    Rune *p;
    int m;
...
Rune tolowerrune(Rune c)
{
    ...
    p = bsearch_(c, __tolower2, nelem(__tolower2) / 3, 3);
    if (p && c >= p[0] && c <= p[1])
        return c + p[2] - 500;
    p = bsearch_(c, __tolower1, nelem(__tolower1) / 2, 2);
    if (p && c == p[0])
        return c + p[1] - 500;
    return c;
}
...
Rune toupperrune(Rune c)
{
    ...
    p = bsearch_(c, __toupper2, nelem(__toupper2) / 3, 3);
    if (p && c >= p[0] && c <= p[1])
        return c + p[2] - 500;
    p = bsearch_(c, __toupper1, nelem(__toupper1) / 2, 2);
    if (p && c == p[0])
        return c + p[1] - 500;
    return c;
}
...
Rune totitlerune(Rune c)
{
    ...
    p = bsearch_(c, __totitle1, nelem(__totitle1) / 2, 2);
    if (p && c == p[0])
        return c + p[1] - 500;
    return c;
}
...
int isalpharune(Rune c)
{
    ...
    if (isupperrune(c) || islowerrune(c))
        return 1;
    p = bsearch_(c, __alpha2, nelem(__alpha2) / 2, 2);
    if (p && c >= p[0] && c <= p[1])
        return 1;
    p = bsearch_(c, __alpha1, nelem(__alpha1), 1);
    if (p && c == p[0])
        return 1;
    return 0;
}
...
int isspacerune(Rune c)
{
    ...
    p = bsearch_(c, __space2, nelem(__space2) / 2, 2);
    if (p && c >= p[0] && c <= p[1])
        return 1;
    return 0;
}
