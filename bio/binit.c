...
static int bioread(Biobuf *bp, void *v, long n)
{
    return read(bp->fid, v, n);
}

static int biowrite(Biobuf *bp, void *v, long n)
{
    return write(bp->fid, v, n);
}

int Binits(Biobuf *bp, int f, int mode, uchar *p, int size)
{
...
void Biofn(Biobuf *bp, int (*f)(Biobuf *, void *, long))
{
    if (f == nil)
